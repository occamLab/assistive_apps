...
from junction_function import *

class Junction_Viz():
    def __init__(self, manual_rotation):
        ...
        self.AR_Edges = {}
        self.dummyidlist = []
        self.g2o_result_path = '/home/juicyslew/catkin_ws/result.g2o'#path.join(top, g2o_result_path)
        self.g2o_data_path = '/home/juicyslew/catkin_ws/data_cp.g2o'#path.join(top, g2o_data_path)
        #self.testlist = []
        self.test_traj = []
                    ...
                    if not line[0] in self.dummyidlist:
                        ...
                        if line[0] >= 587:
                            ...
                            if dist > self.movethreshold:
                                lastpos = line[1:4]
                                self.juncverts.append((count, line[1], line[2], line[3]))
                                print("found junction vertex: " + str(line[0]))
    ...
    def run(self, junctions = None, inds = None):
        ...
        junction_inds = [] #gets actual indices for junctions, not modulod indices.
        for i in junctions:
            junction_inds.append(int(inds[i]))

        junction_data = []
        for i in range(len(junction_inds)):
            test = (traj_data[junction_inds[i], 0], traj_data[junction_inds[i], 1], traj_data[junction_inds[i], 2])
            save = 1;
            for compare in junction_data:
                if distance_calc(compare,test) < self.junc_min_dist:
                    save = 0;
                    print('uh oh')
                    break
            if save:
                junction_data.append((traj_data[junction_inds[i], 0], traj_data[junction_inds[i], 1], traj_data[junction_inds[i], 2]))
        print(len(junction_inds))
        print(len(junction_data))
        junction_data = np.asarray(junction_data)




        junctions_path, = plt.plot(junction_data[:, 0], junction_data[:, 1], junction_data[:, 2], 'ro', label = 'junctions')
        ...
        plt.legend(handles = [new_path, junctions_path]) #old_path,
        plt.xlabel('X')
        plt.ylabel('Y')
        #plt.zlabel('Z')
        plt.title('Junction Plot')
        plt.grid(True)

        plt.show()

if __name__ == "__main__":
    junc_viz = Junction_Viz((0,.085,.012))
    junc_viz.GatherData()
    junc_func = Junction_Function()
    junc_func.vertices = junc_viz.juncverts
    junc_func.run(True)
    junc_viz.run(junc_func.detections, junc_func.inds)
